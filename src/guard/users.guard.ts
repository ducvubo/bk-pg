...
export class UserAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<any> {
    ...
    if (!access_token || !refresh_token) throw new UnauthorizedError('Token không hợp lệ1')
    try {
      ...
      if (!rf_public_key_refresh_token || !rf_public_key_access_token)
        throw new UnauthorizedError('Token không hợp lệ4')
      ...
      if (!dataToken[0] || !dataToken[1]) throw new UnauthorizedError('Token không hợp lệ2')

      const user = await this.usersService.findOneById({ _id: dataToken[0]._id })
      if (!user) throw new UnauthorizedError('Token không hợp lệ 5')
      ...
      console.log(error)
      throw new UnauthorizedError('Token không hợp lệ 3')
    }
  }
}
