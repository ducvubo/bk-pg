...
export class CategoryController {
  ...
  async createCategory(@Body() createCategoryDto: CreateCategoryDto, @User() user: IUser): Promise<CategoryDocument> {
  ...
  @UseGuards(UserAuthGuard)
  async updateCategory(@Body() updateCategoryDto: UpdateCategoryDto, @User() user: IUser): Promise<CategoryDocument> {
    return this.categoryService.updateCategory(updateCategoryDto, user)
  }

  @Get('/home')
  @ResponseMessage('Lấy danh mục trang chủ thành công')
  async findCategoryHome(@Query('limit') limit: number): Promise<CategoryDocument[]> {
  ...
  @UseGuards(UserAuthGuard)
  async restore(@Param('id') _id: string, @User() user: IUser): Promise<CategoryDocument> {
  ...
  @UseGuards(UserAuthGuard)
  async remove(@Param('id') _id: string, @User() user: IUser): Promise<CategoryDocument> {
  ...
  @UseGuards(UserAuthGuard)
  findOne(@Param('id') _id: string): Promise<CategoryDocument> {
